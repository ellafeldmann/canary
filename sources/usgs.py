...
def _load_config():
    ...
    return full["sources"]["usgs"], full["alerting"]
...
def run():
    ...
    for feature in features:
        ...
        if severity < alerting["send_at_severity"]:
            continue
        if store.has_alert_for_event(conn, event_id):
            continue
        ...
        log.info("alert sent for %s (severity %.0f)", external_id, severity)
